...
?>
<div class="login">
    <form action="?page=ervaringedit&id=<?php echo $id; ?>" method="POST">
    ID:<br>
    <input type="text" value="<?php echo $id; ?>" disabled><br>
    Ervaring:<br>
    <select name="ervaring_id">
        <?php
            foreach($ervaringen as $ervaring){
                echo "<option value='".$ervaring['Persoon_Skills_Ervaring_Id']."' ";
                if($post_ervaring_id==$ervaring['Persoon_Skills_Ervaring_Id']){
                    echo "selected";
                }
                echo ">".$ervaring['Omschrijving']."</option>";
            }
        ?>
    </select>
    Uren gewenst:<br>
